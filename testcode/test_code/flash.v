...
`define Read_Status_Reg                 8'h05//register-1
`define Write_Status_Reg                8'h01//register-1
`define Read_Data_Bytes                 8'h03
`define Read_Data_At_Higher_Speed       8'h0B//Fast Read
`define Page_Program                    8'h02
`define Sector_Erase                    8'hD8//64KB Block 
...
always@(*)  begin
    case (state)
        Flash_Idle: 
            ...
            else if( write_enable_req == 1'b1)   //外部直接请求写使能 先进入写使能，自动插入需要进行的写操作
