...
module spi_ip #(
    ...
    parameter   CLK_DIV = 16'd50        // SCLK分频系数（系统时钟周期数/半SCLK周期）
)(
    input   wire                clk,
    input   wire                rst,
...
always @(*) begin
    case (state)
        S_IDLE:
            if (wr_req)
                next_state = S_CLK_HALF;
            else
                next_state = S_IDLE;

        S_CLK_HALF:
            if (clk_cnt == CLK_DIV - 1)
                next_state = S_CLK_EDGE;
            else
                next_state = S_CLK_HALF;

        S_CLK_EDGE:
            if (edge_cnt == 5'd15)
                next_state = S_LAST_HALF;
            else
                next_state = S_CLK_HALF;
...
always @(posedge clk or posedge rst) begin
    if (rst)
        clk_cnt <= 16'd0;
    else if (state == S_CLK_HALF || state == S_LAST_HALF)
...
always @(posedge clk or posedge rst) begin
    if (rst)
        mosi_shift <= 8'd0;
    else if (state == S_IDLE && wr_req)
        mosi_shift <= data_in;      // 锁存发送数据
    else if (state == S_CLK_EDGE) begin
