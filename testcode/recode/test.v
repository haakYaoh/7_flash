    ...
    case (state)
        SPI_IDLE: 
            if( write_req == 1'b1  || read_req == 1'b1)
                ...
                next_state <= SPI_IDLE;
        SPI_DATA:
            if( spi_rev_send_bit_cnt == 4'd7 && spi_clk_inverse_cnt == 1'b1) // 上升沿计数完成
                next_state <= SPI_END;
            else
                next_state <= SPI_DATA;
        SPI_END:
...
always@(posedge sys_clk or negedge rst)    begin
    ...
    else if( spi_clk_inverse_cnt == 1'b1)   // 上升沿计数
...
always @(posedge sys_clk or negedge rst)   begin
    ...
    else if( state == SPI_DATA && spi_clk_inverse_cnt == 1'b0)  // 下降沿移位（输出）
...
always@(posedge sys_clk or negedge rst)    begin
    if( rst == 1'b0)
        spi_clk_reg <= 1'b0;            // 空闲低电平（模式0）
    else if(state == SPI_DATA)
        spi_clk_reg <= ~spi_clk_reg;
    else   
        spi_clk_reg <= 1'b0;            // 空闲低
end
...
always@(posedge sys_clk or negedge rst) begin
    if( rst == 1'b0)
        ...
        spi_mosi_reg <= write_data_reg[7];  // 在下降沿更新后输出新位
    else    
        spi_mosi_reg <= 1'b1;
end
...
always@(posedge sys_clk or negedge rst)    begin
    if( rst == 1'b0)
        read_data_reg <= 1'b0;
    else if(state == SPI_DATA && spi_clk_inverse_cnt == 1'b1)  // 上升沿采样
